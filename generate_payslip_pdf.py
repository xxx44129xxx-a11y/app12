...
def generate_payslip_pdf_bytes(data):
    ...
    row("ค่าทำงานวันหยุด", "", "", f"{data['holiday']:,.2f}", "ขาดงาน", f"{data['absent']:,.2f}")
    row("ค่าล่วงเวลา OT", f"{data['ot_hours']}", "ชม.", f"{data['ot']:,.2f}", "ลากิจ/ป่วย", f"{data['leave']:,.2f}")
    row("เบี้ยขยัน", "", "", f"{data['diligence']:,.2f}", "สาย", f"{data['late']:,.2f}")
    row("ค่าเป้า", "", "", f"{data['target']:,.2f}", "ภาษี", f"{data['tax']:,.2f}")
